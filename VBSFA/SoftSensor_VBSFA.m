...
Ntest = 20;
% 训练数据集
Train_Dataset_X = Data_X((zeta+1): (Ntrain + zeta), :);
Train_Dataset_y = Data_Y((zeta+1): (Ntrain + zeta), :);
% 测试数据集
Test_Dataset_X = Data_X((Ntrain + zeta + 1): (Ntrain + zeta + Ntest), :);
Test_Dataset_y = Data_Y((Ntrain + zeta + 1): (Ntrain + zeta + Ntest), :);

%% 组合数据集
F = [Train_Dataset_X, Train_Dataset_y];
mean_data = zeros(1, size(F, 2));
variance_data = ones(1, size(F, 2));
[F, mean_data, variance_data] = auto(F);


%% 超参数
Factor_Num = 3;
nMaxIter = 200;
dim_X = size(Train_Dataset_X, 2);
dim_Y = size(Train_Dataset_y, 2);

mean_X = mean_data(:, 1:dim_X);
variance_X = variance_data(:, 1:dim_X);
mean_Y = mean_data(:, (dim_X + 1):(dim_X + dim_Y));
variance_Y = variance_data(:, (dim_X + 1):(dim_X + dim_Y));
%% 给先验
% 这里X的组成是P * N(P为X的自变量维度，N为数据条数)
X = F';
piror.K = Factor_Num;
piror.m_s = zeros(piror.K, size(X, 2));
...
[Y_pred, regressor] =  SoftSensorVBSFA(Train_Dataset_X, q, dim_X, dim_Y, mean_X, variance_X, mean_Y, variance_Y);
[Y_pred_test, regressor_test] =  SoftSensorVBSFA(Test_Dataset_X, q, dim_X, dim_Y, mean_X, variance_X, mean_Y, variance_Y);
%% 开始软测量
...
q.m_a = 1.0 * ones(piror.K, size(X, 1));
% piror.m_a = zeros(piror.K, size(X, 1));
...
%% 画ELBO的线
ELBO_list = ELBO_list(2:(iter + 1), :);
ELBO_length = linspace(1, size(ELBO_list, 1), size(ELBO_list, 1));
%ELBO_length = size(ELBO_list, 1);
figure
plot(ELBO_length,ELBO_list,'b','LineWidth',2.5);
end
